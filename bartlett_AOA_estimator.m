function [profile,azimuth_angle,conf,azimuth_angle2,conf2] = bartlett_AOA_estimator(hrList, yawList, pitchList, ...
    ...
    if simple
        ...
        azimuth_angle = rad2deg(betaList(index));
        
        % Find second angle at other side of the plot
        if azimuth_angle<0 %first angle on left side
            [conf2,index2]=max(profile(ceil(end/2)+1:end,1),[],"all");
            azimuth_angle2 = rad2deg(betaList(index2 + ceil(end/2)));
        else %first angle on right side
            [conf2,index2]=max(profile(1:ceil(end/2),1),[],"all");
            azimuth_angle2 = rad2deg(betaList(index2));
        end
    else
        % Angle for 3d not implemented
        % Could potentially make use of findpeaks() ?

        [conf,index] = max(profile(:),[],"all");
        [I_row, I_col] = ind2sub(size(profile),index);
        azimuth_angle = rad2deg(betaList(I_row));
        elevation_angle = rad2deg(gammaList(ngamma-I_col+1));

        % % Find second angle at other side of the plot
        % if azimuth_angle<0 %first angle on left side
        %     [conf2,index2]=max(profile(ceil(end/2):end,:),[],"all");
        %     azimuth_angle2 = rad2deg(betaList(index2 + ceil(end/2)))
        %     %ignore elevation angle..
        % 
        % else %first angle on right side
        %     [conf2,index2]=max(profile(1:ceil(end/2),:),[],"all");
        %     %%[I_row, I_col] = ind2sub(size(profile),index2);
        %     azimuth_angle2 = rad2deg(betaList(index2))
        %     %ignore elevation angle..
        % end
        if azimuth_angle<0 %first angle on left side
            profile2 = profile(ceil(end/2):end,:);
            betaList2 = betaList(ceil(end/2):end);

        else %first angle on right side
            profile2 = profile(1:ceil(end/2),:);
            betaList2 = betaList(1:ceil(end/2));
        end
        [conf2,index2] = max(profile2(:),[],"all");
        [I_row2, I_col2] = ind2sub(size(profile2),index2);
        azimuth_angle2 = rad2deg(betaList2(I_row2));
        elevation_angle2 = rad2deg(gammaList(ngamma-I_col2+1));
    end
end
