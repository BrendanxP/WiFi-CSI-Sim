% %%
data = importRealData_func(folder,tx_name);
% 
% %%
% data = cleanRealData_func(data,tx_mac,tx_pos);
% 
% %%
% data = processRawData_func(data,sc_range,ignore_tx_time);

%% Load all the data (can take a minute)
function data = importDataCSI(folder,sc_range,tx_pos,tx_mac,tx_name,ignore_tx_time)
    ...
    %ignore_tx_time overwrites time of TX with RX when they misalign massively
    arguments
        folder 
        sc_range 
        tx_pos (1,3) = [0,0,0] % default to origin
        tx_mac {mustBeText} = '' % make sure its left empty when not defined
        tx_name {mustBeText} = "tx"
        ignore_tx_time = false % default to false
    end
    % Step 1
    data = importRealData_func(folder,tx_name);
    %Step 2
    [data,success] = cleanRealData_func(data,tx_mac,tx_pos);
    if success
        % Step 3
        data = processRawData_func(data,sc_range,ignore_tx_time);
    end
...
%% Helper functions
%%% IMPORT REAL DATA
function data = importRealData_func(folder,tx_name)
    ...
    for i=1:length(file_names)
        ...
        switch file_type
            case 'dat'
                ...
                if unit == 'tx'
                    % TX name can be TX, TX1, TX2, etc.
                    % Count the num of characters of TX we want plus one to
                    % include the tailing underscore.
                    tx_name_len = length(char(tx_name))+1;
                    if ~strcmp( file_name(5:(4+tx_name_len)), tx_name + "_" )
                        % Means the file is not the TX we want
                        warning("skipped %s",file_name(5:(4+tx_name_len)))
                        continue;
                    end
                    run = file_name((5+tx_name_len):end-4);
                elseif unit == 'rx'
            ...
            case 'csv'
                switch file_name(1:8)
                    ...
                    case 'rx_traje'
                        name_start = 15;
                    otherwise
                        warning("skipped incorrect odom name %s",file_name)
                        continue;
                end
                
                % Remove random underscore "_" in naming of WSR dataset...
                if file_name(end-4) == '_'
                    name_end = 5; % omit the underscore
                else
                    name_end = 4;
                end

                % Define run and parse data
                run = file_name(name_start:end-name_end);
...
function [data,success] = cleanRealData_func(data,tx_mac,tx_pos)
    ...
    for i=1:numel(data)
        ...
        elseif data(i).run(1) == 'f' || data(i).run(1) == 'r'
            ...
            data(i).odom_tx=tx_pos; % Add position TX
        ...
        time_func = @(s) setfield(s,'time',s.tv_sec + s.tv_usec/1e6);
        data(i).csi_rx = arrayfun(time_func, data(i).csi_rx);
        data(i).csi_tx = arrayfun(time_func, data(i).csi_tx);

        % Check if all csi loaded in well and thus exists
        if ~isfield(data(i), 'csi_rx') || ~isfield(data(i), 'csi_tx') || ...
            isempty(data(i).csi_rx) || isempty(data(i).csi_tx)
            success=false;
            warning("csi field not loaded correctly, check the inputs names.")
            continue
        end
        ...
        if ~all(tx_macs == tx_macs(1)) % check if multiple macs exist
            ...
            warning("Filtering on specified mac %s",tx_mac)
            data(i).csi_rx = data(i).csi_rx(tx_macs == string(tx_mac));
        end

        % Fix odom timestamp if it contains sec,nano_sec to sec.nanosec,0
        if isfield(data(i),"odom_rx") && ~isempty(data(i).odom_rx)
            if data(i).odom_rx(:,2) ~= 0
                data(i).odom_rx(:,1) = data(i).odom_rx(:,1) + data(i).odom_rx(:,2)/1e9;
                data(i).odom_rx(:,2) = 0;
            end
        end
...
%%% PROCESS RAW DATA
function data = processRawData_func(data,sc_range,ignore_tx_time)
    for i=1:length(data)
        ...
        % aligning time RX, TX and odom only works when robot timings 
        % are configured properly. Overwrite when needed.
        if ignore_tx_time
            warning("overwriting TX time with RX")
            for j = 1:length(data(i).csi_tx)
                % make time tx equal to rx based on framecount which
                % have already been aligned.
                data(i).csi_tx(j).time = data(i).csi_rx(j).time;
            end
        end
        
        % Separate odom to remove odom entries before and after capture
        % of CSI
        odom_rx_temp = data(i).odom_rx;

        % Get the timestamps of each item
        time_odom = odom_rx_temp(:,1);    % Odometry RX timestamp
        time_rx = [data(i).csi_rx.time];       % CSI RX timestamp
        time_tx = [data(i).csi_tx.time];       % CSI TX timestamp
        
        len = length(time_odom);
        odom_rx = odom_rx_temp(and(time_odom>=time_rx(1),time_odom<=time_rx(end)),:);
        time_odom = odom_rx(:,1);    % Remake odometry RX timestamp after removing items
        
        % Keep track of how much odom data is removed.
        removed = len - length(time_odom);
        %disp(removed)
        ...
        odom_x_int = num2cell(interp1(odom_rx(:,1),odom_rx(:,3),time_rx(time_move),'spline')');
        odom_y_int = num2cell(interp1(odom_rx(:,1),odom_rx(:,4),time_rx(time_move),'spline')');
        ...
        switch size(data(i).odom_rx,2)
            ...
            case 5 
                % Interpolate Z.
                odom_z_int = num2cell(interp1(odom_rx(:,1),odom_rx(:,5),time_rx(time_move),'spline')');
            ...
            case 6
                ...
                [data(i).csi_rx_moving.odom_z_int] = odom_z_int{:};
                % Interpolate Yaw.
                odom_yaw_int = num2cell(interp1(odom_rx(:,1),odom_rx(:,6),time_rx(time_move),'spline')');
