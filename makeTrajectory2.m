function [vx, vy, vz, rz] = makeTrajectory2(traj, numR, num_points)

    % predefine variables
    vx = cell(numR,1); vy = vx; vz = vx; rz = vx;
    

    %Peak velocity = robot constant
    %End time = num_points
    %Size (circle diameter, square
        ...
        switch traj.Type{i}
            ...
            case "circle"
                ...
                if traj.Omnidirectional{i}
                    ...
                    slowitrs = 10; % start and end slower to prevent major slipping and drift
                    temp = ones(1,num_points) * traj.VelocityLin{i};
                    temp(1:slowitrs) = temp(1:slowitrs)/(slowitrs:-1:1);
                    temp(end-slowitrs-1:end) = temp(end-slowitrs-1:end)/(1:slowitrs);
                    vx{i} = temp;
            ...
            case "random-walk"
                ...
                vy{i}(p1+1:p2) = 0;
                vz{i}(p1+1:p2) = 0;
                rz{i}(p1+1:p2) = traj.VelocityRot{i};

                % Part 3
                vx{i}(p2+1:p3) = traj.VelocityLin{i};
                vy{i}(p2+1:p3) = 0;
                vz{i}(p2+1:p3) = 0;
                rz{i}(p2+1:p3) = 0;
